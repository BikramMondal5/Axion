...
export function MarketplacePreview() {
  return (
    <section
      id="marketplace"
      className="relative overflow-hidden bg-black/[0.96] bg-grid-white/[0.02] px-4 py-24 sm:px-6 lg:px-8"
    >
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {agents.map((agent) => {
            ...
            return (
              <Card
                key={agent.name}
                className="group flex flex-col border-slate-800 bg-slate-900/50 p-6 backdrop-blur-sm transition-all hover:border-indigo-500/50 hover:bg-slate-900/80"
              >
                ...
                <Badge className="mb-3 w-fit border-indigo-500/30 bg-indigo-500/10 text-indigo-400">
                  {agent.category}
                </Badge>

                <h3 className="mb-2 text-lg font-semibold text-white">{agent.name}</h3>
                <p className="mb-4 flex-1 text-sm leading-relaxed text-slate-400">{agent.description}</p>

                <div className="flex items-center justify-between border-t border-slate-800 pt-4">
                  <span className="text-sm font-semibold text-indigo-400">{agent.price}</span>
                  <Button size="sm" variant="ghost" className="text-slate-300 hover:bg-white/5 hover:text-white">
                    Try Now
                  </Button>
                </div>
              </Card>
        ...
        <div className="mt-12 text-center">
          <Button variant="outline" className="border-slate-700 bg-transparent text-white hover:bg-white/5">
            View All Agents
          </Button>
        </div>
      </div>
